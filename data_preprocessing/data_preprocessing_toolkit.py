...
#         https://www.skytowner.com/explore/getting_all_weekends_between_two_dates_in_pandas
        
        df["weekend_stay"] = ""
        for index, row in df.iterrows():
            weekend = pd.bdate_range(start=row["date_from"], end=row["date_to"] - timedelta(days=1), freq="C", weekmask="Fri Sat")
#             display(weekend)

            if(len(weekend) == 0):
                df.at[index, "weekend_stay"] = "False"
            else:
                df.at[index, "weekend_stay"] = "True"
    ...
    def aggregate_group_reservations(self, df):
        ...
        mode_columns = group_reservations.groupby('group_id')[self.mode_columns].agg(lambda x:x.value_counts().idxmax())
        ...
        aggregated_group_reservations = pd.merge(sum_columns, mean_columns, on='group_id')
        aggregated_group_reservations = pd.merge(aggregated_group_reservations, mode_columns, on='group_id')
        aggregated_group_reservations = pd.merge(aggregated_group_reservations, first_columns, on='group_id')
        aggregated_group_reservations = aggregated_group_reservations.reset_index().fillna(0)
        
        return pd.concat([non_group_reservations, aggregated_group_reservations])
